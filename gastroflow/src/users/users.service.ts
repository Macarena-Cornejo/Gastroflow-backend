...
export class UsersService {
  ...
  async getEmployees(
    requestUser: RequestUserPayload,
  ): Promise<EmployeeResponse[]> {
    const restaurantId = this.getRestaurantIdFromPayload(requestUser);
    const employees =
      await this.userRepository.getEmployeesByRestaurantId(restaurantId);
  ...
  async createEmployee(
    ...
    const employee = await this.userRepository.getEmployeeByIdAndRestaurantId(
      id,
      restaurantId,
    );
    if (!employee)
  ...
  async updateEmployeeStatus(
    ...
    const updatedEmployee = await this.userRepository.updateUserStatus(
      employee.id,
      isActive,
    );
    if (!isActive) {
  ...
  async resetEmployeePassword(
    ...
    if (
      ...
      throw new BadRequestException(
        'newPassword debe tener entre 8 y 15 caracteres',
      );
    }
  ...
  private async getEmployeeForRequester(
    id: string,
    requestUser: RequestUserPayload,
  ): Promise<User> {
    ...
    const employee = isSuperAdmin
      ...
      : await this.userRepository.getEmployeeByIdAndRestaurantId(
          id,
          this.getRestaurantIdFromPayload(requestUser),
        );
  ...
  private validateCreateEmployeeDto(dto: CreateEmployeeDto): void {
    ...
    if (!dto.lastName?.trim())
      throw new BadRequestException('lastName es requerido');
  ...
  private toUserRole(role: string): UserRole {
    ...
    if (!userRole) {
      throw new BadRequestException(
        'Rol invalido. Usa cocinero, cajero o mesero',
      );
    }
